...
var eurecaClientSetup = function() {
    ...
    // try a client function to be called by server
...
BasicGame.Game.prototype = {
    ...
    create: function () {
        ...
        this.bg = this.add.tileSprite(0, 0, 1200, 600, 'background');
        ...
        */
        ...
        // listen for generateCoin\
        //socket.on('coinPosition',client.generateCoinFromServer.bind(this));
        //socket.on('bloodPosition',client.generateBloodFromServer.bind(this));
        // listen for item generation
        socket.on('generateItem',client.generateItem.bind(this));
        // player2 emit bullet added
        socket.on('counterPartyEmit',client.counterPartyEmit.bind(this));  
        // game end
        socket.on('gameEnd',client.gameEnd.bind(this));
        socket.on('playerDied',client.playerDied.bind(this));
        
        // listen for overlap with items
        //io.emit('coinCollectConfirm',true);
        //socket.on('coinCollectConfirm',client.coinCollectConfirm.bind(this));
        //io.emit('bloodCollectConfirm',true); 
        //socket.on('bloodCollectConfirm',client.bloodCollectConfirm.bind(this));
        
        // server can generate items
        //this.serverCanGenCoins();
        socket.emit('canGenerate',{clientID:123});
        //eurecaClientSetup();
    ...
    updateInformation:function(data){
        ...
        // just update 
            ...
            this.player2.x = data['positionX'];
            this.player2.y = data['positionY'];
            this.player2.direction = data['direction'];
            
            this.player.score = data['myScore'];
            this.player.health = data['myHP'];
            //this.player.playerBulletNumber = data['myBulletNum'];
            // bullet number ??
            
            this.player2.score = data['hisScore'];
            this.player2.healt = data['hisHP'];
    ...
    render: function(){
        ...
        if (this.timer.running) {
            this.game.debug.text(this.formatTime(Math.round((this.timerEvent.delay - this.timer.ms) / 1000)), 565, 605, "#ff0");
        }
        else {
            this.game.debug.text("Game End!", 565, 605, "#0f0");
    ...
    setupPlayer: function(){
        ...
        this.player2 = this.add.sprite(260+50, 100+30, 'phaser');
    ...
    informationDisplay:function(){
        ...
        this.t = this.add.text(20,590,  this.scoreString + this.player.score, { font: "18px Arial", fill: "#ffffff", align: "center" });
        this.t.fixedToCamera = true;
        this.t.cameraOffset.setTo(20, 590);

        // player health
        this.healthInform = this.add.text(150,590, 'HP : '+this.player.health, { font: "18px Arial", fill: "#ffffff", align: "center" });
        this.healthInform.fixedToCamera = true;

        this.speedInform = this.add.text(240,590, 'Speed : '+this.player.speed, { font: "18px Arial", fill: "#ffffff", align: "center" });
        this.speedInform.fixedToCamera = true;

        // bullet inform
        this.bulletInform = this.add.text(370,590, 'Bullet : '+this.player.playerBulletNumber, { font: "18px Arial", fill: "#ffffff", align: "center" });
        this.bulletInform.fixedToCamera = true;
        
        // display information to player2
        this.hisName = this.add.text(800,590,  'Name: ' + this.player2.name, { font: "18px Arial", fill: "#ffffff", align: "center" });
        this.hisName.fixedToCamera = true;
        this.score2 = this.add.text(930,590,  'Score: ' + this.player2.score, { font: "18px Arial", fill: "#ffffff", align: "center" });
        this.score2.fixedToCamera = true;
        //this.score2.cameraOffset.setTo(400, 590);

        // player health
        this.healthInform2 = this.add.text(1050,590, 'HP : '+this.player2.health, { font: "18px Arial", fill: "#ffffff", align: "center" });
    ...
    fire:function(){
            ...
            if(!this.player.alive || this.player.playerBulletNumber <= 0 ){
    ...
    setUpItems:function(){
        ...
        this.bloodPool.createMultiple(BasicGame.NumOfBlood, 'blood');
        ...
        this.enemyPool.callAll('animations.add', 'animations', 'enemyEffect', [0, 1, 2, 3], 10, true);
    ...
    */
    ...
    speedUpEvent:function(player,speedUp){
        ...
        speedUp.kill();
        socket.emit('declareOverlap',{itemType:'speedUp',itemID:speedUp.ID});
        
        // increase player speed, not consider
    ...
    bulletAddEvent:function(player, bulletAdd){
        ...
        socket.emit('declareOverlap',{itemType:'bulletAdd',itemID:bulletAdd.ID});
        bulletAdd.kill();
     ...
     bloodAddToServer:function(player,blood){
        ...
        socket.emit('declareOverlap',{itemType:'blood',itemID:blood.ID});
        blood.kill()
    ...
    collectCoinToServer:function(player, coin){
        ...
        socket.emit('declareOverlap',{itemType:'coin',itemID:coin.ID});
        coin.kill();
   ...
   generateItemHelper:function(type,objectGroup,positionX,positionY,objectID){
    ...
    playerHitEnemy:function(player, enemy){
        ...
        socket.emit('declareOverlap',{itemType:'enemy',itemID:enemy.ID});
        // add effect
    ...
    bulletHitEnemy:function(bullet,enemy){
        console.log('Player destory enemy!');
        // tell server
        socket.emit('hitOther',{hitType:'enemy',itemID:enemy.ID});
        //
        bullet.kill();
        this.bigExplode(enemy);
        enemy.kill();
    ...
    enemyBullletHitPlayer:function(bullet, player2){
        
        console.log('hit conterParty!');
        socket.emit('hitOther',{hitType:'counterParty',itemID:'hisName'});
        
        this.bigExplode(bullet);
        bullet.kill();
    ...
    */
    ...
    setUpGamePad:function(){
        ...
        this.indicator = this.add.sprite(580,605, 'controller-indicator');
